...
public class FileSaver {
    ...
    public static void save(Context c, ScanManager scanManager, String filename) {
        ...
        saveLibraryAverage(c, scanManager, filename);
        saveRawLibrary(c, scanManager, filename);
        saveRaw(c, scanManager, filename);
    }

    private static void saveRawLibrary(Context c, ScanManager scanManager, String filename) {
        //assert isExternalStorageWritable();
        //Creates a new file
        File file = new File(c.getExternalFilesDir(null), "libraryRaw" + filename);
        for (int i = 0; i < scanManager.getNumberOfScans(); i++) {
            ...
            for (String key : scanManager.getAnchorNodes().keySet()) {
                output += " " + index++ + ":" + scanManager.getAnchorNodes().get(key).getRssis().get(i);
            }
            for (float value : scanManager.getMagneticFieldValues().get(i)) {
                output += " " + index++ + ":" + value;
            }
    ...
    public static void saveRaw2(Context c, ScanManager scanManager, String filename) {
        assert isExternalStorageWritable();
        //Creates a new file
        File file = new File(c.getExternalFilesDir(null), "raw" + filename);
     ...
     */
    public static void saveLibraryAverage(Context c, ScanManager scanManager, String filename) {
        assert isExternalStorageWritable();
